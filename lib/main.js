...
export default {
  ...
  detectEngineFromMagicComment(filePath) {
    const fs = require("fs");
    try {
      ...
      for (const line of lines) {
        ...
        if (match) {
          ...
          if (["pdflatex", "xelatex", "lualatex"].includes(engine)) {
            if (atom.config.get("latex-tools.debug")) {
              console.log(`[LaTeX Tools] Detected engine from magic comment: ${engine}`);
            }
            return engine;
          }
        }
      ...
      if (atom.config.get("latex-tools.debug")) {
        console.error("[LaTeX Tools] Failed to read file for magic comment:", error);
      }
    }
    return null;
  },
  ...
  openLatexmkrc() {
    ...
    let latexmkrcPath = possiblePaths[0];
    ...
    if (!fs.existsSync(latexmkrcPath)) {
      try {
        fs.writeFileSync(latexmkrcPath, "", "utf8");
        atom.notifications.addSuccess(`Created ${latexmkrcPath}`);
      } catch (error) {
  ...
  runCompilation(filePath) {
    ...
    const magicEngine = this.detectEngineFromMagicComment(filePath);
