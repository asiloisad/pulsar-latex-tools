...
export default {
  ...
  activate(state) {
    ...
    this.latexPanel = new LaTeXPanel();
    this.logParser = new LogParser();
    this.statusBarView = new StatusBarView(() => this.togglePanel());
    ...
    this.subscriptions.add(
      ...
      atom.commands.add('atom-workspace', {
        'latex-tools:toggle-panel': () => this.togglePanel(),
      }),
      atom.workspace.addOpener(uri => {
        if (uri === 'atom://latex-tools/issues') {
  ...
  compile() {
    ...
    this.setBuildState(filePath, 'building', `Building...`);
    this.statusBarView.setStatus('building', `Building...`);
    ...
    childProcess.on('exit', (code, signal) => {
      ...
      if (code === 0) {
        this.setBuildState(filePath, 'success', `${fileName} built successfully`);
        ...
        this.setBuildState(filePath, 'error', `Build failed (exit code ${code})`);
