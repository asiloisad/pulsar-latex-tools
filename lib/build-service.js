...
module.exports = class BuildService {
  ...
  onDidStartBuild(callback) {
    if (atom.config.get("latex-tools.debug")) {
      console.log(
        "[LaTeX Tools] BuildService: Registered onDidStartBuild callback"
      );
    }
  ...
  onDidFinishBuild(callback) {
    if (atom.config.get("latex-tools.debug")) {
      console.log(
        "[LaTeX Tools] BuildService: Registered onDidFinishBuild callback"
      );
    }
  ...
  onDidFailBuild(callback) {
    if (atom.config.get("latex-tools.debug")) {
      console.log(
        "[LaTeX Tools] BuildService: Registered onDidFailBuild callback"
      );
    }
  ...
  onDidChangeBuildStatus(callback) {
    if (atom.config.get("latex-tools.debug")) {
      console.log(
        "[LaTeX Tools] BuildService: Registered onDidChangeBuildStatus callback"
      );
    }
  ...
  onDidUpdateMessages(callback) {
    if (atom.config.get("latex-tools.debug")) {
      console.log(
        "[LaTeX Tools] BuildService: Registered onDidUpdateMessages callback"
      );
    }
  ...
  onDidChangeCompileOnSave(callback) {
    if (atom.config.get("latex-tools.debug")) {
      console.log(
        "[LaTeX Tools] BuildService: Registered onDidChangeCompileOnSave callback"
      );
    }
   ...
   */
  isAnyBuilding() {
    return Array.from(this.buildingFiles.values()).some(
      (s) => s.status === "building"
    );
  }
  ...
  compile(filePath) {
    ...
    if (!filePath || !filePath.endsWith(".tex")) {
      if (atom.config.get("latex-tools.debug")) {
        console.log(
          "[LaTeX Tools] BuildService: Invalid file path for compile"
        );
      }
      return false;
    }
