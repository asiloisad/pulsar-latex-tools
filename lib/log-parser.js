...
// Pattern for continuation lines (starts with spaces or package name in parens)
const CONTINUATION_PATTERN = /^(?:\s{15,}|\([\w-]+\)\s*)(.+)$/;
...
export default class LogParser {
   ...
   */
  collectContinuationLines(lines, startIndex, packageName = null) {
    const collected = [];
  ...
  parseFatalError(match, lines, lineIndex) {
    ...
    while (nextIndex < lines.length) {
      ...
      if (line.match(/^(!|LaTeX|Package|Class|Output|Over|Under)/) ||
    ...
    return {
      message: {
        ...
        logRange: [[lineIndex, 0], [nextIndex - 1, lines[nextIndex - 1]?.length || 0]]
      },
      nextIndex
    };
  }
  ...
  parseBoxWarning(match, lineIndex) {
    ...
    return {
      ...
      logRange: [[lineIndex, 0], [lineIndex, 0]]
    };
  }
  ...
  parseWarning(match, lines, lineIndex) {
    ...
    const continuation = this.collectContinuationLines(lines, nextIndex, packageName);
    ...
    const excerpt = originText !== 'LaTeX'
